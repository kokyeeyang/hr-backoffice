		...
		switch($strController){
			case 'registration':
				...
				if($bolAuthenticated === true && isset(Yii::app()->user->priv) && in_array(Yii::app()->user->priv, ['admin', 'manager', 'hr'])){
					...
					$arrActionsList = array_merge($arrActionsList, ['showAllCandidates', 'addNewJobOpenings', 'saveJobOpenings', 'showAllJobOpenings', 'deleteSelectedJobOpenings', 'generateLink', 'generateEmail', 'deleteSelectedCandidates', 'viewSelectedCandidate', 'updateSelectedCandidate']);
				}	
			break;
