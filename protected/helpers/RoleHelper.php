...
class RoleHelper {
	...
	public static function GetRole($strController, $bolAuthenticated = false){
		...
		switch($strController){
			...
			case 'onboarding':
				if($bolAuthenticated === true && isset(Yii::app()->user->priv) && in_array(Yii::app()->user->priv, ['admin', 'manager', 'hr'])){
					$arrActionsList = array_merge($arrActionsList, ['addNewOnboardingItem', 'saveOnboardingItem', 'showAllOnboardingItems', 'viewSelectedOnboardingItem', 'deleteOnboardingItem', 'updateOnboardingItem']);
				}
			break;
		}
